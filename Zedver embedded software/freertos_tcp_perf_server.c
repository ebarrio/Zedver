...
{
	u8_t recv_buf[RECV_BUF_BYTES];
	u16_t read_bytes, send_bytes;
	int sock = *((int *)p);
	...
	ComControl = init_struct(0,0,0,0,0);
	u8_t control_in_process = 0;
	u16_t remaining_bytes = 0;
	while (1) {
		/* read a max of RECV_BUF_SIZE bytes from socket */
		if ((read_bytes = lwip_recvfrom(sock, recv_buf, RECV_BUF_BYTES,
						0, NULL, NULL)) < 0) {
		...
		xil_printf("Buffer size: %u\n\r",read_bytes);
		/*xil_printf("Data: %u %u %u %u %u\n\r",recv_buf[0], recv_buf[1], recv_buf[128],recv_buf[129], recv_buf[500]);
		*//* break if client closed connection */
		...
		//xil_printf("Dato recibido: %u \n\r", read_bytes);
		server.total_bytes += read_bytes;
		// Communication struct control
		if(control_in_process == 0){
			ComControl = init_struct(/*Mode:*/ recv_buf[0],/*Peticion:*/ recv_buf[1],
					/*BytesRAW:*/uchars_to_ushort(recv_buf[2],recv_buf[3]),
					/*BytesHisto:*/uchars_to_ushort(recv_buf[4],recv_buf[5]),
		...
		if(ComControl.ready_to_send){
			...
			if ((send_bytes = write(sock, ComControl.pay, send_bytes)) < 0) {
				xil_printf("%s: ERROR responding to client echo request. received = %d, written = %d\r\n",
							__FUNCTION__, read_bytes, send_bytes);
